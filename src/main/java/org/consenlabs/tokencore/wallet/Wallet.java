...
public class Wallet {
     ...
     * 自己加的，为了方便业务逻辑处理，不会存储到钱包本地文件中去
     */
    private String balanceCoin = "0.0";
    /**
     * 标识值
     * 自己加的，为了方便业务逻辑处理，不会存储到钱包本地文件中去
     */
    ...
    boolean delete(String password) {
        return keystore.verifyPassword(password) && WalletManager2.getInstance().generateWalletFile(keystore.getId()).delete();
    }
