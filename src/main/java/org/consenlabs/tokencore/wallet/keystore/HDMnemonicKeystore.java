...
//        byte[] bytes = crypto.decrypt(password);
//    }
     ...
     * @return
     */
    public String getMainPrivateKey(List<String> mnemonics) {
        DeterministicSeed seed = new DeterministicSeed(mnemonics, null, "", 0L);
        DeterministicKeyChain keyChain = DeterministicKeyChain.builder().seed(seed).build();

        NetworkParameters networkParameters = metadata.isMainNet() ? MainNetParams.get() : TestNet3Params.get();

        DeterministicKey mainAddressKey = keyChain.getKeyByPath(BIP44Util.generatePath(mnemonicPath + "/0/0"), true);
        BigInteger privkeybtc = mainAddressKey.getPrivKey();
        ECKey ecKey = ECKey.fromPrivate(privkeybtc);

        return ecKey.getPrivateKeyEncoded(networkParameters).toString();
    }

    private List<ChildNumber> parsePath(@Nonnull String path) {
