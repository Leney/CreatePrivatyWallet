...
public class Identity {
    ...
    private Identity(IdentityKeystore keystore) {
        ...
        for (String walletId : this.keystore.getWalletIDs()) {
            wallets.add(WalletManager2.getInstance().findWalletById(walletId));
        }
    }
    ...
    public void deleteIdentity(String password) {
        ...
        if (WalletManager2.getInstance().cleanKeystoreDirectory()) {
            WalletManager2.getInstance().clearWalletList();
    ...
    public Wallet deriveOneWallet(String chainType, String password) {
        String mnemonic = exportIdentity(password);
        List<String> mnemonics = Arrays.asList(mnemonic.split(" "));
        Wallet wallet;
        switch (chainType) {
    ...
    private static Identity tryLoadFromFile() {
        try {
            File file = new File(WalletManager2.getInstance().getDefaultKeyDirectory(), FILE_NAME);
    ...
    private Wallet deriveBitcoinWallet(List<String> mnemonicCodes, String password, String segWit) {
        ...
        return WalletManager2.getInstance().createWallet(keystore);
    }
